#> Iceball tag
tag @a[nbt={SelectedItem:{id:"minecraft:snowball",tag:{CustomModelData:3}}}] add HoldBB
...
execute as @e[tag=GreenBBArrow,tag=!ThrownBall] at @s run data modify entity @s Owner set from entity @a[team=Green,scores={throwsb=1..},limit=1,sort=nearest] UUID
execute as @e[tag=RedBBArrow,tag=!ThrownBall] at @s run data modify entity @s Owner set from entity @a[team=Red,scores={throwsb=1..},limit=1,sort=nearest] UUID
...
tag @e[type=arrow,tag=BBottle,tag=!ThrownBall] add ThrownBall
...
execute as @e[type=arrow,tag=BBArrow,tag=BottleHit] at @s run particle block glass ~ ~ ~ 0.3 0.3 0.3 0.1 30 force
