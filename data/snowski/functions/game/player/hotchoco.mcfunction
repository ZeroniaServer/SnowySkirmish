#> Prevent bug where the effects work when you've fallen.
scoreboard players reset @a[scores={knocktime=95..}] drinkchoco

#> Knocked players
execute as @a[tag=Knocked,scores={drinkchoco=1..}] run scoreboard players set @s Health 50
...
#> Non-knocked players
execute as @a[tag=!Knocked,scores={drinkchoco=1..}] run scoreboard players add @s Health 50
execute as @a[tag=!Knocked,scores={drinkchoco=1..}] at @s run playsound minecraft:item.honey_bottle.drink master @a ~ ~ ~ 1 2
execute as @a[tag=!Knocked,scores={drinkchoco=1..}] at @s run particle minecraft:heart ~ ~0.5 ~ 0.2 0.3 0.2 0.1 5
