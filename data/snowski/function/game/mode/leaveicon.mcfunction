...
execute as @a[team=!Lobby,team=!Spectator,tag=YouSure] unless items entity @s inventory.22 diamond_hoe[custom_data={ExitIcon:1b}] run scoreboard players set @s leavegame 1

execute as @a[team=!Lobby,team=!Spectator] unless items entity @s inventory.22 diamond_hoe[custom_data={ExitIcon:1b}] run tag @s add YouSure
execute as @a[team=!Lobby,team=!Spectator,tag=YouSure] run clear @s diamond_hoe[custom_data={ExitIcon:1b}]
execute as @a[team=!Lobby,team=!Spectator,tag=YouSure] run item replace entity @s inventory.13 with diamond_hoe{CustomModelData:6,ExitIcon:1b,HideFlags:2,display:{Name:'[{"text":"[Are you sure?]","italic":false,"color":"red"}]',Lore:['[{"text":"","italic":false}]','[{"text":"Click on this icon again to leave the game!","italic":false,"color":"white"},{"text":"","italic":false,"color":"dark_purple"}]','[{"text":"","italic":false,"color":"dark_purple"}]']}}